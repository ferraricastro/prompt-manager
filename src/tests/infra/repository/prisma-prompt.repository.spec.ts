...
type PromptDelegateMock = {
  ...
  findMany: jest.MockedFunction<
    (args: {
      ...
      whre?: {
        OR: Array<{
          title?: { constains: string; mode: 'insensitive' };
          content?: { constains: string; mode: 'insensitive' };
        }>;
      };
    }) => Promise<Prompt[]>
  >;
...
describe('PrismaPromptRepository', () => {
  ...
  describe('findMany', () => {
    it('deve ordenar o createdAt desc e mapear os resultados', async () => {
      const now = new Date();
      const input = [
        {
          id: '1',
  ...
  describe('searchMany', () => {
    it('deve buscar por termos vazio e não enviar o where', async () => {
      const now = new Date();
      const input = [
        {
          id: '1',
